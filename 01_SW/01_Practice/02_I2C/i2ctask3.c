...
float calc_temp(signed int temp_raw, unsigned int bit_info)
{
    float temp_val;
    if (bit_info==16){
        temp_val = (float)temp_raw / 128.0f;
    }
    else if(bit_info==13){
        temp_val = (float)(temp_raw >> 3) / 16.0f;
    }
...
void main()
{
    ...
    for(;;){
        ...
        i2cMasterWriteRead(1,&i2cAddr,&msbRegAddr,1,rdata,2);
        msb=rdata[0];
        lsb=rdata[1];
        temp_raw=(msb << 8)+lsb;
        temp_val=calc_temp(temp_raw, 16);
        printf("MSB(RAW)=%#x, LSB(RAW)=%#x, TEMP(RAW)=%#x\r\n", msb, lsb, temp_raw);
        printf("The Value of Current Temperature is %.3f C.\r\n", temp_val);
        __delay_ms(1000);
        ...
        lsb=rdata[1];
        temp_raw=(msb << 8)+lsb;
        temp_val=calc_temp(temp_raw, 13);
        printf("MSB(RAW)=%#x, LSB(RAW)=%#x, TEMP(RAW)=%#x\r\n", msb, lsb, temp_raw);
        printf("The Value of Current Temperature is %.3f C.\r\n\r\n", temp_val);
        __delay_ms(1000);
    }
