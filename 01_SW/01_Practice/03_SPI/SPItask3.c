...
void spiReadTemp(const unsigned int select){
    ...
    unsigned char commandTempRead = 0b01010000;
    
//    unsigned char config13bit = 0b00000000;
//    unsigned char config16bit = 0b10000000;
    unsigned char config16bit = 0x80;
    unsigned char config13bit = 0x00;
    ...
    if(select==1){
        ...
        CS_PIN = 0;
        spiReadTest(&commandTempRead, tempBuf, 2);
        temp_raw = (tempBuf[0] << 8) + tempBuf[1];
        temp_val = temp_raw / 128.0f;
        printf("Raw of Current Temperature is %#x\r\n", temp_raw);
        printf("Value of Current Temperature is %.3f C\r\n\r\n", temp_val);
        CS_PIN = 1;
        ...
        CS_PIN = 0;
        spiReadTest(&commandTempRead, tempBuf, 2);
        temp_raw = ((tempBuf[0] << 8) + tempBuf[1]) >> 3;
        temp_val = temp_raw / 16.0f;
        printf("Raw of Current Temperature is %#x\r\n", temp_raw);
        printf("Value of Current Temperature is %.3f C\r\n\r\n", temp_val);
        CS_PIN = 1;
